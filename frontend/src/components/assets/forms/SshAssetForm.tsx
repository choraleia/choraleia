...
const SshAssetForm = React.forwardRef<SshAssetFormHandle, Props>(
  (
    ...
    // Track the asset ID to detect actual asset changes vs just refetches
    const assetIdRef = React.useRef(asset?.id);

    useEffect(() => {
      // Only reset form when asset ID actually changes (not on refetch of same asset)
      if (assetIdRef.current === asset?.id && assetIdRef.current !== undefined) {
        return;
      }

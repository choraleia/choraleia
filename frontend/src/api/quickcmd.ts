...
interface ListResponse { commands: QuickCommandDTO[]; total: number }
...
export async function createQuickCommand(payload: { name: string; content: string; tags: string[] }): Promise<QuickCommandDTO> {
...
export async function updateQuickCommand(id: string, payload: Partial<{ name: string; content: string; tags: string[] }>): Promise<QuickCommandDTO> {
  const resp = await fetch(`${API_BASE}/${id}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
  const j: Response<QuickCommandDTO> = await resp.json();
...
export async function reorderQuickCommands(ids: string[]): Promise<QuickCommandDTO[]> {
  const resp = await fetch(`${API_BASE}/reorder`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ ids }) });
  const j: Response<ListResponse> = await resp.json();
